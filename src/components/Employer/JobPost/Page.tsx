...
export default function JobPosting() {
  ...
  const validateStep = (currentStep: any) => {
    ...
    if (currentStep === 1) {
      ...
      } else if (new Date(formData.deadline).getTime() < new Date().setHours(0, 0, 0, 0)) {
  ...
  const handleDropdownChange = (name: string) => (selected: any) => {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <p>{formData.jobDifficulty.join(", ")}</p>
        </div>
        <div>
          <h3 className="font-semibold">Project Length</h3>
          <p>{formData.projectLength.join(", ")}</p>
        </div>
        <div>
